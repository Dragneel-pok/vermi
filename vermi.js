...
bot.on("message", (message) => {
  ...
  //another say  or TELL
  if (
    message.content.toLowerCase().startsWith(prefix + "tell") &&
    ["672695020100386846"].includes(message.author.id)
  ) {
  ...
  if (message.content.toLowerCase().startsWith(prefix + "apply")) {
    if (message.channel.id !== "729940234107879464")
      return message
        .reply(
          "pls use <#729940234107879464> channel! command usage not allowed here"
        )
        .then((m) => m.delete({ timeout: 3000 }));
  ...
  if (message.content.toLowerCase().startsWith(prefix + "gfxapply")) {
    ...
    var applyembed = new Discord.MessageEmbed()
      .setColor("#f16798")
      .setDescription(
        `${message.author} You applied for the <@&730312225692319794> special role (please give the best u made with **Dragneel** details in it below) Management will get back to you soon!Good Day`
      );
    message.channel.send({ embed: applyembed });
    var receiveembed = new Discord.MessageEmbed()
      .setColor("#ff255e")
 ...
 > \`Joining for\`-- **\`Esports | Casual Rank Playing| Time pass\`**
      ...
      if (message.member.hasPermission("MANAGE_ROLES")) {
        if (message.mentions.members.size < 1) {
          ...
          var gfxembed = new Discord.MessageEmbed()
            ...
            .setDescription(
              sayMessage +
                ` <@&677465656966381588> requirements were not met as per requirements mentioned in <#677560689732223027>! **please make to read it properly and then apply again** `
            );
          message.guild.channels.cache
            .get("729943660124176494")
            .send(`${user.toString()}`)
  ...
  if (message.content.toLowerCase().startsWith(prefix + "howhead")) {
    ...
    var headembed = new Discord.MessageEmbed()
      ...
      .setColor("#aef105")
      .addField(
        "**HEADS**--are SERVER MANAGERS",
        "|How enrolled > well depends on server needs + starts when **DRAGNEEL** thinks needed| so stay tuned! keep checking announcements for staying updated"
      )
      .setFooter(message.author.username, message.author.displayAvatarURL())
      .setThumbnail(
        "https://cdn.discordapp.com/attachments/726134541638697042/729221373695754240/tenor.gif"
      );
    message.channel
      .send({ embed: headembed })
      .then((m) => m.delete({ timeout: 9000 }));
  }
  ...
  if (message.content.toLowerCase().startsWith(prefix + "howmod")) {
    ...
    if (message.member.hasPermission("MANAGE_GUILD"))
      var modembed = new Discord.MessageEmbed()
        ...
        .setColor("#aef105")
        .addField(
          "** Aꜱᴛʀᴀʟ ᴍᴏᴅS **--are Server Moderators",
          "|How enrolled > Well giving someone MOD means trusting one like a family| `TOP chat rank holders are kept on evaluation` by **Management** so keep loving our family and u might just get that REP u want [:negative_squared_cross_mark: **Never ask for MOD randomly**- u will be given strike for free|:woozy_face: ]"
        )
        .setFooter(message.author.username, message.author.displayAvatarURL())
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/726134541638697042/733207306019536977/tenor.gif"
        );
    message.channel
      .send({ embed: modembed })
  ...
  if (message.content.toLowerCase().startsWith(prefix + "howgfx")) {
    ...
    if (message.member.hasPermission("MANAGE_SERVER"))
      var modembed = new Discord.MessageEmbed()
        ...
        .setColor("#aef105")
        .addField(
          "**Dragy's Artist **--are gfx DESIGNERS",
          "|How enrolled > Head over to <#729940234107879464> and just do `+gfxapply` make sure to do what's written there"
        )
        .setFooter(message.author.username, message.author.displayAvatarURL())
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/726134541638697042/729228621297745990/tenor_5.gif"
        );
    message.channel
      .send({ embed: modembed })
      .then((m) => m.delete({ timeout: 15000 }));
  ...
  if (message.content.toLowerCase().startsWith(prefix + "howvip")) {
    ...
    if (message.member.hasPermission("MANAGE_SERVER"))
      var modembed = new Discord.MessageEmbed()
        ...
        .setColor("#aef105")
        .addField(
          "**Royals **--are like super OP people worthy of being honoured",
          "|How given > Well Royals are people quite close to Dragneel | are recognised everywhere | are just vibed people"
        )
        .setFooter(message.author.username, message.author.displayAvatarURL())
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/726134541638697042/733687829242314853/mavy_charmed.gif"
        );
    message.channel
      .send({ embed: modembed })
      .then((m) => m.delete({ timeout: 15000 }));
 ...
 `
        ...
        .setFooter(message.author.username, message.author.displayAvatarURL())
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/726134541638697042/733687835416330271/mavuu_more_happy.gif"
        );
    message.channel
      .send({ embed: modembed })
  ...
  if (message.content.toLowerCase().startsWith(prefix + "howpartner")) {
    ...
    if (message.member.hasPermission("MANAGE_SERVER"))
      var modembed = new Discord.MessageEmbed()
        ...
        .setFooter(message.author.username, message.author.displayAvatarURL())
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/726134541638697042/733688140090441748/wat_the_hell.gif"
        );
    message.channel
      .send({ embed: modembed })
      .then((m) => m.delete({ timeout: 3000 }));
  ...
  if (message.content.toLowerCase().startsWith(prefix + "howyt")) {
    ...
    if (message.member.hasPermission("MANAGE_GUILD"))
      var modembed = new Discord.MessageEmbed()
        .setAuthor(bot.user.username)
        .setTitle(
          " How `☢│ Yᴏᴜᴛᴜʙᴇʀꜱ 〢`|`☢│ Fᴀᴍᴇᴅ ʏᴏᴜᴛᴜʙᴇʀS 〢`Get their REP in our home"
        )
        .setColor("#aef105")
        .setDescription(
          `
 ...
 `
        ...
        .setFooter(message.author.username, message.author.displayAvatarURL())
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/726134541638697042/734226748849520690/ram_ram.gif"
        );
    message.channel
      .send({ embed: modembed })
  ...
  if (message.content.startsWith(prefix + `warn`)) {
    if (!message.member.hasPermission(`KICK_MEMBERS`))
      return message.reply(`Don't be dumb please leave warn and all to mods!`);
    if (!reason) {
      return message.channel.send(
        `Please Provide a reason in order to warn.e.g - \`${config.prefix}warn <user> <reason>\``
      );
    }
    if (!msguser) {
    ...
    var warnEmbed = new Discord.MessageEmbed()
      .setColor("#c7c5c5")
      .setDescription(
        `<a:emoji_96:725928974877720677> **Wizard \`${msguser.user.username}\`   has been warned for**-- **\`${reason}\`**!. `
      );
    message.channel.send(warnEmbed);
    message.delete();
    {
      var dmwarnembed = new Discord.MessageEmbed().setDescription(
  ...
  if (message.content.startsWith(prefix + `unwarn`)) {
    ...
    if (!unreason) {
      return message.channel.send(
        `Please Provide a reason in order to unwarn.e.g - \`${config.prefix}unwarn <user> <reason>\``
      );
    }
    var guild = message.guild;
    var unwarnEmbed = new Discord.MessageEmbed()
      .setColor(`BLUE`)
      .setDescription(
        `<a:emoji_96:725928974877720677> **Wizard    has been UNwarned for**-- **\`${reason}\`**!. `
      );
    ...
    var dmembed = new Discord.MessageEmbed()
      .setColor("GREEN")
      .setDescription(
        `**You have been unwarned from **\`${message.author.tag}\` in \`${message.guild.name}\` for \`${unreason}\`!`
      );
  ...
  if (message.content.toLowerCase().startsWith(prefix + "kick")) {
    if (!message.member.hasPermission("KICK_MEMBERS")) {
      return message.reply(
        "Not having perms, still trying to kick someone | **;(**"
      );
    }
    ...
    if (!member) {
      return message.reply(
        "umm @mention should be of a valid wizard of our guild"
      );
    }
    if (!member.kickable) {
      return message
        .reply(
          "deng this role might be higher,do i have kick perm? |can't kick the user"
        )
    ...
    if (!reason) {
      member
        .kick(`No Reason - by ${message.author}`)
        .catch((error) =>
          message.reply(
            `Sorry ${message.author} couldn't  be kicked cause : ${error}`
          )
        );
      message.channel.send({
        embed: {
        ...
        .send({
          embed: {
            ...
            description:
              ...
              reason +
              "`" +
              ` from ${message.guild.name}  if you think you where kicked by mistake please contact **Support Team**.`,
          },
        })
        .catch((error) =>
          message.channel.send(
            `⚠ Unable to alert ${member} of reason \`${error}\`.`
          )
        )
        .then((message) => {
          member
            .kick(`${reason} - On command of ${message.author}`)
            .catch((error) =>
              message.reply(
                `sollie ${message.author} couldn't kick maybe cause ${error}`
              )
            );
          member
            .send({
              embed: {
                ...
                description:
                  ...
                  reason +
                  "`" +
                  " from `Dragneel's Server`. if you think you were kicked by mistake please contact **Support Team**.",
              },
            })
            .catch((error) => console.log(error));
  ...
  if (message.content.toLowerCase().startsWith(prefix + "ban")) {
    if (!message.member.hasPermission("BAN_MEMBERS")) {
      return message.reply(
        "Not having perms, still trying to ban someone | **;(**"
      );
    }
    ...
    if (!member.kickable) {
      return message
        .reply(
          "nuuu this role might be higher,do i have kick perm? |can't kick the user"
        )
        .then((m) => m.delete({ timeout: 6000 }));
    }
    ...
    if (!reason) {
      member
        .ban(`No Reason - by ${message.author}`)
        .catch((error) =>
          message.reply(
            `Sorry ${message.author} couldn't  be banned cause : ${error}`
          )
        );
      message.channel.send({
        embed: {
      ...
      member
        .send({
          embed: {
            ...
            description:
              ...
              reason +
              "`" +
              ` from ${message.guild.name}  if you think you where banned by mistake please contact **Support Team**.`,
          },
        })
        .catch((error) =>
          message.channel.send(
        ...
        .then((message) => {
          member
            .ban(`${reason} - On command of ${message.author}`)
            .catch((error) =>
              message.reply(
                `sollie ${message.author} couldn't kick maybe cause ${error}`
              )
            ...
            .send({
              embed: {
                ...
                description:
                  ...
                  reason +
                  "`" +
                  " from `Dragneel's Server`. if you think you were banned by mistake please contact **Support Team**.",
              },
            })
            .catch((error) => console.log(error));
          message.delete({ timeout: 5000 });
  ...
  //TEMPMUTE-----------------------
  if (message.content.toLowerCase().startsWith(prefix + "tmute")) {
    if (!message.member.hasPermission("MUTE_MEMBERS"))
      return message.reply("Nuuu u cant do that |**:^**");

    let person = message.guild.member(
      message.mentions.users.first() || message.mentions.members.get(args[0])
    );
    ...
    if (!time) {
      return message
        .reply(
          `ahhh | the format goes as \`${prefix}mute <time> <reason>\` ^0^`
        )
        .catch((error) => console.log(error));
    }
    ...
    if (!reason) {
      ...
      message.channel.send(
        `${person.toString()} has been muted|\`no reason\` |\`${ms(ms(time))}\``
      );
      message.delete();
    } else {
      person.roles.remove(mainrole.id);
      ...
      message.channel.send(
        `${person.toString()} has been muted|\`${reason}\` |\`${ms(ms(time))}\``
      );
      message.delete();
    }
  ...
  //MUTE---------------------------------------
  if (
    message.content.toLowerCase().startsWith(prefix + "mute" || prefix + "m")
  ) {
    if (!message.member.hasPermission("MUTE_MEMBERS"))
      return message.reply("Nuuu u cant do that |**:^**");
    var reason = args.slice(1).join(" ");
  ...
  //UNMUTE ----------------------
  if (
    message.content.toLowerCase().startsWith(prefix + "unmute" || prefix + "um")
  ) {
    if (!message.member.hasPermission("MUTE_MEMBERS"))
      return message.reply("Nuuu u cant do that |**:^**");
  ...
  if (message.content.toLowerCase().startsWith(prefix + "ping")) {
    if (message.channel.id !== "684657636150345769") return null;
    {
      ...
      message.channel.send(pingEmbed).then((m) => m.delete({ timeout: 8000 }));
    }
  }
  //uptime
  if (message.content.toLowerCase().startsWith(prefix + "uptime")) {
    message.channel.send(
      `**Mavis vermilion** was last awakened **${(bot.uptime / 60000).toFixed(
        0
      )} minutes** ago!`
    );
  }
